...
class OLGClass:
    ...
    def baseline(self):
        ...
        par.s = np.full(par.simT, 0.98) # Probability of surviving (becoming old)
        par.p_ini = 1 # initial population
        par.p_young = [par.p_ini] + [0] * (par.simT - 1) # The young initial population
        par.p_old = [par.p_ini*par.s[0]/(1+par.n)] + [0] * (par.simT - 1) # The old initial population
    ...
    def simulate_after_s(par,sim,t,s):
        ...
        sim.C1[t] = (1 - par.tau_w) * sim.w[t] * (1.0 + par.n) ** t * (1 - s)
